...
def openai_chat(db, chat_id=None, user_message="", context_type="", user_id=None, title=""):
    file_url = ""
    ...
    if context_type == "Image/Generate":
        ...
        if chat_id is not None:
            ...
            conversation = ChatData(user_id=user_id, title=title, messages=json.dumps(existing_messages),
                                    created_at=datetime.utcnow())
            db.add(conversation)
        db.commit()
        ...
        reply = parsed_response.get("choices")[0].get("message").get("content")

        assistant_reply_data = {"role": "assistant", "content": reply}
        existing_messages.append(assistant_reply_data)
        if len(title) == 0:
            title = user_message
        if chat_id is not None:
...
async def openai_new_chat_completion(request: Request, db: SessionLocal = Depends(get_db)):
    try:
        ...
        reply, conversation, file_url = openai_chat(db, None, user_message, context_type, user_id, title)
...
async def openai_chat_completion(request: Request, db: SessionLocal = Depends(get_db),
    ...
    try:
        ...
        if images is not None and len(images) > 0:
            ...
            return data
        reply, conversation, file_url = openai_chat(db, chat_id, user_message, context_type)
    except Exception as e:
        return JSONResponse(
            content={
                "status": "failed",
